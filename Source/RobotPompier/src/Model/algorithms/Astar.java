...
public class Astar extends Algorithm {
	...
	public ArrayList<Cell> findBestWay(Map map, Robot robot, Cell cellToReach) {
		...
		openset.add(start);
		gScore.put(start, 0);
		hScore.put(start, start.distance(cellToReach) * robot.getRobotType().getMoveType().getCost(0, start.getTerrainType()));
		...
		while (!openset.isEmpty()) {
			...
			for (Cell c: current.getListCellulesAdjacentes()) {
				...
				if (!openset.contains(c)) {
					openset.add(c);
					hScore.put(c, c.distance(cellToReach) * robot.getRobotType().getMoveType().getCost(0, c.getTerrainType()));
